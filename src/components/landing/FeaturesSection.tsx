          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8">
            {additionalFeatures.map((feature, index) => (
              <RevealStagger key={feature.title} index={index + 5} staggerMs={80}>
                <div className="flex flex-col items-center text-center p-5 sm:p-6 rounded-xl border border-border bg-card shadow-soft-sm transition-colors duration-300">
...
function FeatureCard({ feature }: { feature: (typeof features)[0] }) {
  return (
    <div className="relative h-full">
      <div
        className={cn(
          "relative h-full rounded-2xl p-6 sm:p-7 lg:p-8 border transition-colors duration-300",
